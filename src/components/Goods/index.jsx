const Product = (props) => {
    const { product, deleteProduct, buttonText } = props
    ...
    return (
        <div className="bg-indigo-50 mt-10 py-6 px-8 rounded-md shadow-md flex justify-between">
            ...
            <div className="flex flex-col items-start">
                ...
                <a className="hover:underline cursor-pointer bg-indigo-200 text-indigo-800 font-semibold rounded-sm px-4 py-2 shadow" onClick={() => buttonText(product.id)}>Добавить</a>
            </div>
        </div>
    )
}
